...
public interface DependenciaRepository extends JpaRepository<Dependencia, Long> {
    ...
    @Query(value = "select ud.id as id_usuario_dependencia , ud.rut '' as cargo, usr.first_name nombre,usr.last_name apellidos, pusr.nombre as perfil, ud.estado estado from UsuarioDependencia ud "+
                "inner join dependencia d on ud.dependencia_id  = d.id "+
